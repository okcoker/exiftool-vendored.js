...
type ExpandedDateTags = {
	[K in keyof Tags]:
		| (Tags[K] extends number ? number
			: Tags[K] extends ExifDateTime ? ExifDate | ExifDateTime
			: Tags[K])
		| string;
};
...
export const DefaultMaxProcs = Math.max(
	1,
...
export class ExifTool {
	...
	async extractBinaryTagToBuffer(
		...
		if (Buffer.isBuffer(result)) {
			...
			throw new Error(
				'Unexpected result from BinaryToBufferTask: ' +
