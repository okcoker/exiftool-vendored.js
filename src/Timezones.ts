...
function dtToMs(s: Maybe<string>, defaultZone?: string): Maybe<number> {
    return map(
        ExifDateTime.fromExifStrict(s, defaultZone),
    ...
    return firstDefinedThunk([
        ...
        () =>
            map(
                ...
                (value) =>
                    map(
                        tzHourToOffset(Array.isArray(value) ? value[0] : value),
                        (tz) => ({
                            tz,
                            src: 'TimeZoneOffset',
                        }),
...
export function inferLikelyOffsetMinutes(deltaMs: number): number {
    return TzBoundaryMinutes *
        Math.floor(deltaMs / MinuteMs / TzBoundaryMinutes);
}
...
export function extractTzOffsetFromUTCOffset(t: {
    ...
    const GPSDateTimeStamp = gpsStamps.length === 2
        ? gpsStamps.join(' ')
        : undefined;
