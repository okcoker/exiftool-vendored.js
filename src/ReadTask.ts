import { BatchCluster, path as _path, tzlookup } from './deps.ts';
...
export class ReadTask extends ExifToolTask<Tags> {
    ...
    parse(data: string, err?: Error): Tags {
        ...
        if (this.degroup) {
            Object.keys(this._raw).forEach((keyWithGroup) => {
                this._tags[this.tagName(keyWithGroup)] =
                    this._raw[keyWithGroup];
            });
        } else {
    ...
    private extractTzOffset() {
        map(
            firstDefinedThunk([
                ...
                () => {
                    if (
                        !this.invalidLatLon && this.lat != null &&
                        this.lon != null
                    ) {
                        try {
    ...
    private parseTag(tagNameWithGroup: string, value: any): any {
        ...
        try {
            ...
            const tz = tagName.includes('UTC') || tagName.startsWith('GPS')
                ? 'UTC'
                : this.tz;

            if (typeof value === 'string' && tagName.includes('DateTime')) {
                const d = ExifDateTime.fromExifStrict(value, tz) ??
            ...
            this.addError(
                `Failed to parse ${tagName} with value ${
