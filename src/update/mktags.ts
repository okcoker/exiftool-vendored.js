import { BatchCluster, fs, globSync, path, ProgressBar } from '../deps.ts';
...
class Tag {
        ...
        return (
            TypeOverrides[this.base] ??
                (this.valueTypes.length === 0
                    ? 'string'
                    : this.valueTypes.join(' | '))
        );
    }

    get sortBy() {
        return (
            -(this.required ? 1e8 : this.important ? 1e4 : 1) *
            this.values.length
        );
    }
    ...
    example(): string {
        ...
        if (this.tag.endsWith('Contact')) {
            return exampleToS(['Dohncha Ringmanumba']);
        }
...
async function readAndAddToTagMap(file: string) {
    try {
        ...
        seenFiles.push(file);
        const importantFile = file.toString().toLowerCase().includes(
            'important',
        );
...
Promise.all(files.map((file) => readAndAddToTagMap(file)))
    .then(async () => {
        ...
        console.log(
            `Parsing took ${elapsedMs}ms (${
                (elapsedMs / files.length).toFixed(
                    1,
                )
            }ms / file)`,
        );
        ...
        tagWriter.write(
            [
                ...
                '',
                `// Autogenerated by "yarn mktags" by ExifTool ${version} on ${
        ...
        const tagsByPctPop = times(
            ...
            (pct) =>
                tags.filter((tag) =>
                    tag.values.length / files.length > pct / 100.0
                )
                    .length,
        );
        ...
        console.log(
            '\nInternal error count: ' +
                exiftool['batchCluster'].internalErrorCount,
        );
        return exiftool.end();
    })
    .catch((err) => {
