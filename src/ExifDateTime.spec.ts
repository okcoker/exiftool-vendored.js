...
describe('ExifDateTime', () => {
	...
	describe('example with no tzoffset and UTC default', () => {
		...
		it('Renders a UTC Date assuming the current timezone offset', () => {
			expect(dt.toDate().toISOString()).to.eql(
				'2011-01-23T18:19:20.000Z',
			);
		});
	});
	...
	describe('example with tzoffset and millis', () => {
		...
		it('Renders a Date assuming the forced timezone offset', () => {
			const d = edt.toDate();
			expect([d.getUTCFullYear(), d.getUTCMonth() + 1, d.getUTCDate()]).to
				.eql([
					2013,
					12,
					30,
		...
		it('Renders a UTC Date assuming the current timezone offset', () => {
			...
			expect(edt.toDate().toISOString()).to.eql(
				'2013-12-30T08:04:15.079Z',
			);
		});
	});
	...
	describe('parsing empty/invalid input', () => {
		...
		examples.forEach((bad) => {
			...
			it(bad + ' for SubSecCreateDate', () => {
				expect(ExifDateTime.fromEXIF('SubSecCreateDate')).to.eql(
					undefined,
				);
			});
		});
